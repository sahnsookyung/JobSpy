...
if __name__ == "__main__":
    ...
    jobs = scrape_jobs(site_name="tokyodev", results_wanted=10, location="Tokyo", japanese_requirements=[JapaneseLevel.NONE, JapaneseLevel.BASIC], applicant_locations=[ApplicantLocation.APPLY_FROM_ABROAD], seniorities=[Seniority.JUNIOR, Seniority.INTERMEDIATE])
    print(jobs.head())
    jobs.to_csv("tokyodev.csv", quoting=csv.QUOTE_NONNUMERIC, escapechar="\\", index=False) # to_excel
    # jobs = scrape_jobs(site_name="japandev", results_wanted=5)
    # print(jobs.head())
